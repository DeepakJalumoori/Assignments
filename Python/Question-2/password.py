...
def passcheck(password,old_pass):
     for i in range(len(old_pass)):
          if password == old_pass[i]:
            ...
            break
def consecutive(password, username):
    for i in range(len(password) - 2):
        # Check for sequences of consecutive characters
        if ord(password[i]) == ord(password[i+1]) - 1 == ord(password[i+2]) - 2:
            print("Password should not contain three consecutive characters in sequence.")
            return True

        for j in range(len(username) - 2):
            if password[i:i+3] == username[j:j+3]:
                print("Password should not contain three consecutive characters from the username.")
                return True
    return False
...
pass1 = input("Enter old password 1: ")
pass2 = input("Enter old password 2: ")
pass3 = input("Enter old password 3: ")
